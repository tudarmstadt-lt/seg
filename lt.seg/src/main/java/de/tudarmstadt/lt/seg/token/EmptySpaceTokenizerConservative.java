...
import static de.tudarmstadt.lt.seg.SegmentType.NUMBER;
import static de.tudarmstadt.lt.seg.SegmentType.WORD;
 ...
 *
 */
public class EmptySpaceTokenizerConservative implements ITokenizer {
	...
	private boolean getNext(){
		...
		while(_cp > 0){
			...
			if(!SegmentationUtils.charTypeIsEmptySpace(current_type) && SegmentationUtils.charTypeIsEmptySpace(next_type))
				break;
//			if(WORD_LOWERCASE.allowedCharacterTypes().contains(current_type) && WORD_UPPERCASE.allowedCharacterTypes().contains(next_type))
//				break;
			if(WORD.allowedCharacterTypes().contains(current_type) && !WORD.allowedCharacterTypes().contains(next_type))
				break;
			if(!WORD.allowedCharacterTypes().contains(current_type) && WORD.allowedCharacterTypes().contains(next_type))
				break;
            if(NUMBER.allowedCharacterTypes().contains(current_type) && !NUMBER.allowedCharacterTypes().contains(next_type))
                break;
            if(!NUMBER.allowedCharacterTypes().contains(current_type) && NUMBER.allowedCharacterTypes().contains(next_type))
                break;
            if(!(WORD.allowedCharacterTypes().contains(current_type) || NUMBER.allowedCharacterTypes().contains(current_type)))
                break;
